"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/add/operator/toPromise");
var Card = (function () {
    function Card() {
    }
    return Card;
}());
exports.Card = Card;
var Transaction = (function () {
    function Transaction() {
    }
    return Transaction;
}());
exports.Transaction = Transaction;
var CardService = (function () {
    function CardService(http) {
        this.http = http;
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        this.cardurl = 'http://localhost:58202/api/';
    }
    CardService.prototype.getCardDetails = function () {
        return this.http.get(this.cardurl + 'Credit/GetDetails')
            .toPromise()
            .then(function (response) {
            var det = response.json();
            console.log(det);
            return det;
        })
            .catch(this.handleError);
    };
    CardService.prototype.create = function (creditDetails) {
        return this.http
            .post('http://localhost:58202/api/Credit/save/', creditDetails, { headers: this.headers })
            .toPromise()
            .then(function (res) { return res.json().data; })
            .catch(this.handleError);
    };
    CardService.prototype.saveTransaction = function (cardTransaction) {
        return this.http
            .post('http://localhost:58202/api/Credit/savetransaction/', cardTransaction, { headers: this.headers })
            .toPromise()
            .then(function (res) { return res.json().data; })
            .catch(this.handleError);
    };
    CardService.prototype.getTransactionDetails = function (cardid) {
        return this.http.get(this.cardurl + 'Credit/GetCardDetails/' + cardid)
            .toPromise()
            .then(function (response) {
            var det = response.json();
            console.log(det);
            return det;
        })
            .catch(this.handleError);
    };
    CardService.prototype.handleError = function (error) {
        console.error('An error occurred', error); // for demo purposes only
        return Promise.reject(error.message || error);
    };
    return CardService;
}());
CardService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], CardService);
exports.CardService = CardService;
//# sourceMappingURL=card.service.js.map